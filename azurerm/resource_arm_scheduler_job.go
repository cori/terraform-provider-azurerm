				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"interval": {
							...
							Optional: true,
							Default:  "00:00:30",
							//alidateFunc: internal.ValidateIso8601Duration,
						},
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"week_days": { //used with weekly
							Type:          schema.TypeSet,
							Optional:      true,
							ConflictsWith: []string{"recurrence.0.month_days"},
							MinItems:      1,
						...
						"month_days": { //used with monthly, -1, 1- must be between 1/31
							Type:          schema.TypeSet,
							Optional:      true,
							ConflictsWith: []string{"recurrence.0.week_days"},
							MinItems:      1,
							Elem:          &schema.Schema{Type: schema.TypeInt},
							Set:           set.HashInt,
						},
		...
		Elem: &schema.Resource{
			Schema: map[string]*schema.Schema{
				...
				"url": {
					...
					DiffSuppressFunc: supress.CaseDifference,
					//ValidateFunc:   arm
					//todo validate is a url, in validators file?
					//
				},
		...
		/*if monthly := schedule.MonthlyOccurrences; monthly != nil {
			...
			for _, e := range *monthly {
				slice = append(slice, map[string]interface{}{
					"day":        "Sunday", //string(e.Day),
					"occurrence": 1,        //int(*e.Occurrence),
				})

				log.Printf("[DEBUG] Deleting Scheduler Job %q", e)

			}
			block["monthly_occurrences"] = slice
		}*/
	}
